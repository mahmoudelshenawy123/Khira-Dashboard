...
const ItemsList = () => {
  ...
  const [isActivating,setIsActivating] =useState<[Boolean]>([false])
    ...
    function getAllItems(){
        setIsLoading(true)
        axiosConfig.get(`/orders/all-orders?page=${currentPage}&phone=${searchTerm}&order_number=${orderNumberSearch}`,{
          headers: {
    ...
    function changeActiveStatus(userId:String ,index:any){
      ...
      setIsActivating(modifiedItems)
        axiosConfig.put(`/user/change-user-status/${userId}`,{
          headers: {
              "Authorization": `Bearer ${localStorage.getItem('token')}`
          }
      }).then((res)=>{
  ...
  useEffect(()=>{
    getAllItems()
  },[currentPage,searchTerm,orderNumberSearch])
  return (
    <>
      <KTCard>
        <ItemsListHeader 

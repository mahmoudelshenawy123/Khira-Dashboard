...
const ItemsList = () => {
  ...
  function getAllItems(){
      setIsLoading(true)
      axiosConfig.get(`/advertisement/all-advertisments-with-pagination?page=${currentPage}&name=${searchTerm}`,{
        headers: {
  ...
  function deleteItem(deltedId:String ,index:any){
    ...
    setIsDeleting(modifiedDelete)
      axiosConfig.delete(`/advertisement/delete-advertisment/${deltedId}`,{
        headers: {
            "Authorization": `Bearer ${localStorage.getItem('token')}`
        }
    }).then(res=>{
